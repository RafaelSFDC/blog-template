...
const getDashboardStats = createServerFn({ method: 'GET' }).handler(async () => {
  ...
  const [unreadMessages] = await db.select({ value: count() }).from(contactMessages).where(eq(contactMessages.status, 'new'))
  const [totalViews] = await db.select({ value: sql<number>`sum(${posts.viewCount})` }).from(posts)
  
  const latestPosts = await db.select().from(posts).orderBy(desc(posts.updatedAt)).limit(5)
  const popularPosts = await db.select().from(posts).orderBy(desc(posts.viewCount)).limit(5)
            ...
            <div className="grid gap-4">
               ...
               <Link to="/dashboard/messages" className="island-shell group flex items-center justify-between rounded-2xl bg-card border-3 border-border/50 p-6 text-foreground no-underline transition-all hover:border-rose-500/50 hover:bg-muted/50 shadow-zine-sm">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <p className="text-xs font-black uppercase tracking-widest text-muted-foreground">Inbox</p>
               ...
               <Link to="/dashboard/settings" className="island-shell group flex items-center justify-between rounded-2xl bg-card border-3 border-border/50 p-6 text-foreground no-underline transition-all hover:border-primary/50 hover:bg-muted/50">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        ...
                        <h3 className="display-title text-xl text-primary">Settings</h3>
                     </div>
                  </div>
                  <ArrowRight size={20} className="text-muted-foreground opacity-40 group-hover:opacity-100 transition-opacity" />
               </Link>
