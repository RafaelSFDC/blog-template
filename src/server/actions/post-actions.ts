...
function parsePostStatus(status: string | null | undefined): PostInput["status"] {
  return postStatusSchema.parse(status);
}
...
export const updatePost = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      const previousStatus = parsePostStatus(existingPost.status);
      const revisionSource = shouldTriggerPublishedWebhook(previousStatus, data.status)
        ? "publish"
        : "manual";
      await createPostRevision({
        postId: data.id,
      ...
      if (shouldTriggerPublishedWebhook(previousStatus, data.status)) {
        await captureFirstPostPublishedIfNeeded({
          actorDistinctId: session.user.email,
          actorUserId: session.user.id,
          postId: data.id,
...
async function transitionPostWorkflow(input: {
  ...
  const now = new Date();
  const previousStatus = parsePostStatus(existing.status);
  let nextStatus: PostInput["status"] = previousStatus;
  ...
  const publishedAt = resolvePostPublishedAt(nextStatus, input.scheduledFor, existing.publishedAt);
  const scheduledAt = resolvePostScheduledAt(nextStatus, input.scheduledFor);
  ...
  if (input.action === "publish" && shouldTriggerPublishedWebhook(previousStatus, nextStatus)) {
    await captureFirstPostPublishedIfNeeded({
...
export const getPostPreviewData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      teaserMode: teaserModeSchema.parse(latestRevision?.teaserMode ?? post.teaserMode ?? "excerpt"),
      status: postStatusSchema.parse(latestRevision?.status ?? post.status),
