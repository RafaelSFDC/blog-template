...
import { pageRevisions, pages } from "#/server/db/schema";
...
export const getPagePreviewData = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      ...
      seoNoIndex: latestRevision?.seoNoIndex ?? page.seoNoIndex,
      isPremium: latestRevision?.isPremium ?? page.isPremium,
      teaserMode: teaserModeSchema.parse(latestRevision?.teaserMode ?? page.teaserMode),
      status: pageStatusSchema.parse(latestRevision?.status ?? page.status),
