...
async function createStarterPost(actorUserId: string) {
  const starterSlug: string =
    typeof STARTER_POST_TEMPLATE.slug === "string" && STARTER_POST_TEMPLATE.slug.length > 0
      ? STARTER_POST_TEMPLATE.slug
      : "welcome-to-your-publication";
  const existing = await db.query.posts.findFirst({
    where: eq(posts.slug, starterSlug),
  });
  ...
  const starterPostValues: typeof posts.$inferInsert = {
    title: STARTER_POST_TEMPLATE.title,
    slug: starterSlug,
    excerpt: STARTER_POST_TEMPLATE.excerpt,
    content: STARTER_POST_TEMPLATE.content,
    metaTitle: typeof STARTER_POST_TEMPLATE.metaTitle === "string" ? STARTER_POST_TEMPLATE.metaTitle : null,
    metaDescription:
      typeof STARTER_POST_TEMPLATE.metaDescription === "string"
        ? STARTER_POST_TEMPLATE.metaDescription
        : null,
    ogImage: typeof STARTER_POST_TEMPLATE.ogImage === "string" ? STARTER_POST_TEMPLATE.ogImage : null,
